        ...
        try {
            ...
            if (conn != null) {
                ...
                ResultSet rs = stmt.executeQuery(" SELECT DISTINCT  P.person_nbr FROM patient_diagnosis PD INNER JOIN user_mstr UM ON  PD.created_by = UM.user_id INNER JOIN person P ON PD.person_id = P.person_id INNER JOIN patient_status PS ON P.person_id = PS.person_id INNER JOIN patient_status_mstr PSM ON PS.patient_status_id = PSM.patient_status_id INNER JOIN provider_mstr PM ON UM.provider_id = PM.provider_id WHERE PD.icd9cm_code_id IN ('291.2', 
							...
							'F19.929',
							'F19.239',
							'F19.99') AND (PM.provider_subgrouping2_id ='41CA9A4B-004A-4478-9F38-54061DE3EA32' OR (PM.last_name LIKE 'Adams%' AND PM.first_name LIKE 'David%') OR (PM.last_name LIKE 'Brooklyn%' AND PM.first_name LIKE 'John%') OR (PM.last_name LIKE 'Fisher%' AND PM.first_name LIKE 'Pat%') OR (PM.last_name LIKE 'Inker%' AND PM.first_name LIKE 'Rachel%') OR (PM.last_name LIKE 'Sirois%' AND PM.first_name LIKE 'Michael%') OR (PM.last_name LIKE 'Stein%' AND PM.first_name LIKE 'Heather%') OR (PM.last_name LIKE 'Warnken%' AND PM.first_name LIKE 'Wayne%')OR (PM.last_name LIKE 'Willingham%' AND PM.first_name LIKE 'Jennifer%')) AND (P.last_name NOT LIKE 'Test%' OR P.last_name NOT LIKE 'Pooh%')");
                while (rs.next()){
