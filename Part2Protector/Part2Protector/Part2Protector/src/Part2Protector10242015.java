    ...
    */    
    ...
    public class msgProcessor implements Runnable{
        ...
        for (File f:CCDFilesToUpload){
            try {
                    if(f.renameTo(new File(CCDUploadFileDir+f.getName())))
                    {
        ...
        for (File f:ADTFilesToUpload){
            try {
                    ...
                    if(f.renameTo(new File(ADTUploadFileDir+f.getName())))
                    {
    ...
    public static List getMRNList(){
        ...
        try {
            ...
            if (conn != null) {
                ...
                ResultSet rs = stmt.executeQuery(" SELECT DISTINCT  P.person_nbr FROM patient_diagnosis PD INNER JOIN user_mstr UM ON  PD.created_by = UM.user_id INNER JOIN person P ON PD.person_id = P.person_id INNER JOIN patient_status PS ON P.person_id = PS.person_id INNER JOIN patient_status_mstr PSM ON PS.patient_status_id = PSM.patient_status_id INNER JOIN provider_mstr PM ON UM.provider_id = PM.provider_id WHERE PD.icd9cm_code_id IN ('291.2', '291.81', '292.0', '292.11', '292.81', '292.84', '292.89', '292.9', '303.00', '303.90', '304.00', '304.10', '304.20', '304.21', '304.22', '304.23', '304.3', '304.31', '304.32', '304.33', '304.80', '305.00', '305.20', '305.30', '305.30', '305.40', '305.5', '305.50', '305.51', '305.52', '305.53', '305.6', '305.60', '305.61', '305.62', '305.63', '305.70', '305.90', '305.91', '305.92', '305.93') AND (PM.provider_subgrouping2_id ='41CA9A4B-004A-4478-9F38-54061DE3EA32' OR (PM.last_name LIKE 'Adams%' AND PM.first_name LIKE 'David%') OR (PM.last_name LIKE 'Brooklyn%' AND PM.first_name LIKE 'John%') OR (PM.last_name LIKE 'Fisher%' AND PM.first_name LIKE 'Pat%') OR (PM.last_name LIKE 'Inker%' AND PM.first_name LIKE 'Rachel%') OR (PM.last_name LIKE 'Sirois%' AND PM.first_name LIKE 'Michael%') OR (PM.last_name LIKE 'Stein%' AND PM.first_name LIKE 'Heather%') OR (PM.last_name LIKE 'Warnken%' AND PM.first_name LIKE 'Wayne%')OR (PM.last_name LIKE 'Willingham%' AND PM.first_name LIKE 'Jennifer%')) AND (P.last_name NOT LIKE 'Test%' OR P.last_name NOT LIKE 'Pooh%')");
                while (rs.next()){
